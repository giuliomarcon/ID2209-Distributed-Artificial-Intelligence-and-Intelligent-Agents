...
species Guest skills: [moving, fipa] {
	...
	reflex targetAtTinderTable when: status = 23 and location distance_to (tablePositions[tableIndexUsed] + {tableRadius, 0}) <= 2 and !empty(informs) and
	(informs[0].sender = communicationPartener) {
		message m <- informs[0];
	...
	// +++++++++++++++++++++++++ fsm functioning ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	reflex closePendingApproces when: !empty(informs) {
		message m <- informs[0];
